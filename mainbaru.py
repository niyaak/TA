...
output_video_filename = "hasil_navigasi_adaptif.mp4"
...
while True:
    ...
    # NEW: Variabel baru untuk logika adaptif
    is_sharp_curve_detected = False
    target_path_poly_for_vis = None
    current_target_offset_cm = DESIRED_LANE_WIDTH_CM / 2.0 # Default target
    ...
    if np.sum(mask_resized_to_orig == TEPI_CLASS_VALUE) < MIN_ROAD_PIXELS_TO_OPERATE:
        ...
        if poly_func is not None:
            ...
            # --- NEW: Logika Adaptif untuk Tikungan & Penentuan Jalur Target ---
            # Cek apakah ini tikungan tajam (hanya jika mode belok dan polyfit adalah kurva orde 2)
            if current_gps_navigation_mode == "BELOK" and len(poly_func.coeffs) == 3:
                coeff_a = poly_func.coeffs[0] # Koefisien kuadratik (a) dari ax^2+bx+c
                if abs(coeff_a) > SHARP_CURVE_COEFF_A_THRESHOLD:
                    is_sharp_curve_detected = True
                    # Kurangi target offset agar mobil berbelok lebih tajam ke sisi dalam
                    current_target_offset_cm = (DESIRED_LANE_WIDTH_CM / 2.0) * SHARP_CURVE_ADJUSTMENT_FACTOR

            # Buat fungsi polinomial untuk jalur target (untuk visualisasi)
            target_offset_pixels = current_target_offset_cm / CM_PER_PIXEL
            target_path_poly_for_vis = poly_func + target_offset_pixels
            # --- END NEW LOGIC ---

            try:
                # --- MODIFIED: Logika Input Fuzzy yang Telah Disesuaikan ---
                ...
                # 1. Hitung jarak terukur aktual dari pusat mobil ke tepi kiri
                measured_jt_cm = (center_x_frame - reference_x_abs) * CM_PER_PIXEL

                # 2. Hitung dJT berdasarkan perubahan jarak terukur aktual
                djt_cm_val = 0
                if prev_jt_cm_for_djt_calc is not None:
                    djt_cm_val = measured_jt_cm - prev_jt_cm_for_djt_calc
                prev_jt_cm_for_djt_calc = measured_jt_cm # Simpan pengukuran aktual untuk frame berikutnya

                # 3. Hitung nilai JT 'palsu' untuk diinput ke sistem fuzzy.
                #    Ini 'menipu' sistem fuzzy agar menargetkan `current_target_offset_cm`
                #    alih-alih nilai 'normal' (30cm) yang ter-hardcode dalam Membership Function-nya.
                offset_correction = current_target_offset_cm - FUZZY_SYSTEM_NORMAL_JT_CM
                jt_for_fuzzy_input = measured_jt_cm - offset_correction

                # 4. Lanjutkan dengan clipping dan input ke fuzzy
                ...
                display_jt_cm = f"{measured_jt_cm:.1f}" # Tampilkan jarak terukur aktual
    ...
    # MODIFIED: Menambahkan info "Target Offset"
    ...
    elif "Tajam" in status_info_for_decision: keputusan_color = (0, 165, 255) # Oranye untuk tikungan tajam
    ...
    if target_path_poly_for_vis is not None:
        ...
        for y_p in range(y_draw_line_start_abs, y_draw_line_end_abs - 1, -LINE_SEGMENT_STEP_Y):
            ...
            try:
                ...
                if 0 <= x_p < orig_w: # Gambar hanya jika di dalam frame
